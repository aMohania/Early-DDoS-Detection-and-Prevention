...
from .detectAttack import Entropy
diction = {}
ent_obj = Entropy()
set_Timer = False
defendDDOS=False
...
class l3_switch (EventMixin):
  ...
  def _handle_openflow_PacketIn (self, event):
        ...
        for k,v in diction.iteritems():
          for i,j in v.iteritems():
            if j >=50:
              ...
              print ("\n",str(diction))
              print ("\n",datetime.datetime.now(),": Blocked Port Number  : ", str(i), " of switch: h", str(k))
    ...
    if isinstance(packet.next, ipv4):
      ...
      if ent_obj.value <0.5: # thresghold value change kari hai
        preventing()
